...
db_path = os.path.join(os.getenv('INTERIM_DATA_DIR'), 'observing.db')

connect_to_db(os.path.join(db_path))
...
results_dir = os.path.join(os.getenv('PROCESSED_DATA_DIR'), f'gpfa_observing')
...
for s in tqdm(Session.select()):
    project_and_save(s.id, s.best_gpfa_model, s.trial_trains)
